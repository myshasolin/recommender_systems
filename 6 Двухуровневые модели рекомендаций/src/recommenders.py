...
class MainRecommender:
    ...
    def __init__(self, data, N=5, als_recommend=False, item_item_recommend=False, num_user=False):
        ...
        if item_item_recommend is not False:
            self.model = self.ItemItem_fit(csr_matrix(self.user_item_matrix, dtype='float64'))

        if num_user is not False:
            self.num_user = num_user
            self.recommender = self.get_recommend(self.num_user, self.model, csr_matrix(self.user_item_matrix), N=self.N)
    ...
    @staticmethod
    def ItemItem_fit(user_item_matrix, K=5, num_threads=4):
        model = ItemItemRecommender(K=K, num_threads=num_threads)
    ...
    def get_recommend(self, user, model, sparse_user_item, N=5):
    ...
    def extend_with_top_popular(self, recommendations, N=5):
        ...
        if len(recommendations) < N:
            popular_items = [item_id for item_id in self.overall_top_purchases if item_id not in recommendations]
            recommendations.extend(popular_items[:N-len(recommendations)])
    ...
    def generate_recommendations(self):
        ...
        for user in users:
            recs = self.get_recommend(user, self.model, csr_matrix(self.user_item_matrix), N=self.N)
            extended_recs = self.extend_with_top_popular(recs, self.N)
            recommendations[user] = extended_recs
